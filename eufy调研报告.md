...
sequenceDiagram
    ...
    L->>G: 候选事件理解场景进入红队
    G-->>L: 反证、重复功能和风险
    L->>F: interrupt：候选场景晋级审批
    F-->>L: resume：批准、补研、修改或终止
    L->>F: interrupt：Demo 结果与最终建议审批
    F-->>L: resume：建议立项、补研、修改、不建议立项或终止
```
...
2. **候选场景晋级**：红队评审后，用户选择批准某个候选进入 Demo、补研、修改、淘汰或全部不立项；
3. **最终建议审批**：用户查看 Demo 结果、风险、未知假设和成功指标后，决定建议立项、补充验证、不建议立项或终止，之后才生成正式飞书文档。
...
展示最终产品提案或不建议立项结论、被淘汰场景及原因、Demo 结果、尚未验证的假设，以及 AI 方法和传统方法的对照结果。
...
保存研究任务、来源状态、证据卡片、用户问题、候选场景、专家评分、人工决策和版本记录。多维表格是协作与审计账本，不与后端证据库重复存储全部原始内容。

### 7.3 飞书审批与消息

在研究 Brief 确认、候选场景晋级和最终建议三个节点发起人工审批。LangGraph 在这些节点保存 Checkpoint 并暂停；飞书将批准、补研、修改、淘汰或终止等结构化决定传回后端，主图从原位置恢复，只重跑受影响步骤。
