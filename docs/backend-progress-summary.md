...
> 项目生命周期、统一资料链路、用户研究、竞品 A2A 与生态层综合、通用资料恢复、共享 Evidence Retrieval、设备能力图、AI 原生 Research Brief、多轮追问、真实生态机会、AI Native Gate、技术可行性 Agent 和 Security Policy Compiler 已经完成；主图会在 dry-run 策略编译后停在等待策略验证。策略验证、商业 v2、红队、Demo 与飞书仍未完成，因此系统还不能自动完成一整轮 AI 原生生态定义与验证。
...
| LangGraph AI 原生主路径 | Brief Gate、并行用户/竞品生态研究、Evidence Readiness、生态机会、AI Native Human Gate、技术可行性、Security Policy Compiler、Checkpoint、定向修订和 Source Recovery 恢复 | 可以展示真实节点和门禁状态；策略编译后停在等待 Policy Verification，当前 HTTP 流程仍不会自动启动整张图 |
...
- Security Policy Verification、商业 v2 和红队 Prompt；
