"""通过 Model Gateway 执行竞品用户评价 Prompt 的 A2A 专家 Adapter。"""
...
from app.agents.competitor.user_review_contracts import CompetitorUserReviewModelOutput
from app.agents.competitor.user_review_prompt import COMPETITOR_USER_REVIEW_PROMPT_KEY
...
class CompetitorUserReviewModelSpecialistAdapter:
    adapter_type = "competitor_user_review_internal_model"
    ...
    def __init__(
        ...
        *,
        ...
        validator: CompetitorUserReviewOutputValidator | None = None,
    ) -> None:
        ...
        self.validator = validator or CompetitorUserReviewOutputValidator()
    ...
    async def execute(
        ...
        try:
            ...
            prompt = self.prompt_registry.resolve(COMPETITOR_USER_REVIEW_PROMPT_KEY)
        ...
        try:
            result = await self.model_gateway.generate(
                ModelRequest(
                    ...
                    response_model=CompetitorUserReviewModelOutput,
        ...
        try:
            return self.validator.validate(
                ...
                request=invocation.request,
                evidence_context=evidence_context,
                output=CompetitorUserReviewModelOutput.model_validate(result.output),
            )
        except CompetitorUserReviewValidationError as exc:
            raise RuntimeGatewayError(
    ...
    def _eligible_context(invocation: A2ASpecialistInvocation) -> AgentEvidenceContext:
        products = set(invocation.request.product_scope)
        eligible = [
            item
            for item in (
            ...
            and item.claim_type == EvidenceClaimType.USER_OPINION.value
        ...
        return AgentEvidenceContext(
            ...
            context_hash=CompetitorUserReviewModelSpecialistAdapter._context_hash(eligible),
        )
