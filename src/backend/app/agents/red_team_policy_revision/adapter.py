"""Internal model adapter for Red Team Policy Revision v2."""
...
from app.agents.red_team_policy_revision.prompt import RED_TEAM_PROMPT_KEY
...
class RedTeamModelAgentAdapter:
    adapter_type = "red_team_policy_revision_internal_model"
    ...
    def __init__(
        ...
        *,
        ...
        validator: RedTeamOutputValidator | None = None,
        context_builder: RedTeamContextBuilder | None = None,
    ) -> None:
        ...
        self.validator = validator or RedTeamOutputValidator()
    ...
    async def execute(self, invocation: AgentInvocation) -> object:
        ...
        if missing:
            raise RuntimeGatewayError(
                ...
                "Red Team lacks required upstream Artifacts.",
                agent_run_id=invocation.agent_run_id,
                retryable=False,
                details={"missing_agent_types": missing},
            )
        challenges = [
            RedTeamChallenge.model_validate(item)
            for item in invocation.task.scope.get("challenges", [])
        ]
        upstream = {item: context.upstream_artifacts[item.value] for item in _REQUIRED_UPSTREAM}
        evidence_index = [
            {
        ...
        previous = context.upstream_artifacts.get("previous_red_team")
        try:
            model_id = await self.selection_resolver.resolve(
                invocation.task.project_id, ResearchAgentType.RED_TEAM
            )
            prompt = self.prompt_registry.resolve(RED_TEAM_PROMPT_KEY)
            rendered = prompt.render(
                {
                    ...
                    "challenges_json": json.dumps(
                        [item.model_dump(mode="json") for item in challenges],
                        ensure_ascii=False,
                        sort_keys=True,
                    ),
                    ...
                    "policy_json": upstream[ResearchAgentType.SECURITY_POLICY].model_dump_json(),
                    "verification_json": upstream[
                        ResearchAgentType.POLICY_VERIFICATION
                    ].model_dump_json(),
                    "commercial_json": upstream[
                        ResearchAgentType.COMMERCIAL_EVALUATION
                    ].model_dump_json(),
                    "previous_red_team_json": (
                        previous.model_dump_json() if previous is not None else "null"
                    ),
            ...
            raise RuntimeGatewayError(
                RuntimeErrorCode.DEPENDENCY_MISSING,
                "Red Team lacks model selection or Prompt configuration.",
                agent_run_id=invocation.agent_run_id,
                retryable=False,
        ...
        try:
            result = await self.model_gateway.generate(
                ModelRequest(
                    ...
                    response_model=RedTeamModelOutput,
                    timeout_seconds=min(
        ...
        try:
            return self.validator.validate(
                invocation.task,
                context,
                RedTeamModelOutput.model_validate(result.output),
                challenges,
            )
    ...
    async def _prepare_context(self, invocation: AgentInvocation) -> AgentContext:
        ...
        if self.context_builder is None or context.research_handoff is None:
            raise RuntimeGatewayError(
                RuntimeErrorCode.DEPENDENCY_MISSING,
                "Red Team workflow lacks its bounded context builder.",
                agent_run_id=invocation.agent_run_id,
                retryable=False,
            )
