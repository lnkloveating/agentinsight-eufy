from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path
from tempfile import TemporaryDirectory
from typing import Any

from fastapi.testclient import TestClient

REPOSITORY_ROOT = Path(__file__).resolve().parents[1]
BACKEND_ROOT = REPOSITORY_ROOT / "src" / "backend"
ENVIRONMENT_PATH = BACKEND_ROOT / ".env"
sys.path.insert(0, str(BACKEND_ROOT))

from app.application.model_gateway import ModelCatalog
from app.core.config import Settings
from app.main import create_app
from smoke_competitor_discovery_live import _create_project
from smoke_user_research_live import _expect


def run_live_smoke(model_id: str) -> dict[str, Any]:
    if not ENVIRONMENT_PATH.is_file():
        raise RuntimeError("src/backend/.env is required for the live smoke test")
    base_settings = Settings(_env_file=ENVIRONMENT_PATH)
    ModelCatalog.from_json(
        base_settings.model_catalog_json,
        default_model_id=base_settings.default_model_id,
    ).require_enabled(model_id)

    with TemporaryDirectory(prefix="agentinsight-competitor-onboarding-") as temp_root:
        temp_path = Path(temp_root)
        settings = base_settings.model_copy(
            update={
                "app_env": "smoke-test",
                "database_url": f"sqlite+aiosqlite:///{temp_path / 'smoke.db'}",
                "auto_create_schema": True,
                "model_credentials_env_file": str(ENVIRONMENT_PATH),
                "source_storage_root": str(temp_path / "sources"),
                "source_processing_workspace_root": str(temp_path / "processing"),
            }
        )
        application = create_app(settings)
        with TestClient(application) as client:
            project_id = _create_project(client, model_id)
            search = _expect(
                client.post(
                    f"/api/v1/projects/{project_id}/source-discovery/searches",
                    json={
                        "query": (
                            "Ring Battery Doorbell Pro official exact model smart doorbell"
                        ),
                        "intent": "competitor_candidate",
                        "provider_id": "tavily",
                        "max_results": 5,
                        "include_domains": ["ring.com"],
                        "exclude_domains": [],
                        "requested_by": "backend-smoke-test",
                        "purpose": "Find an exact public competitor product candidate.",
                    },
                ),
                201,
                "run targeted live competitor search",
            )
            if search["status"] != "succeeded" or not search["candidates"]:
                raise RuntimeError(
                    "targeted Tavily search did not return candidates: "
                    f"status={search['status']} error_code={search['error_code']}"
                )
            artifact = _expect(
                client.post(
                    f"/api/v1/projects/{project_id}/agents/competitor-discovery",
                    json={
                        "search_discovery_run_ids": [search["search_discovery_run_id"]],
                        "minimum_candidates": 1,
                    },
                ),
                200,
                "run targeted competitor discovery",
            )
            if not artifact["proposals"]:
                raise RuntimeError(
                    "targeted discovery safely blocked because no exact model was found: "
                    f"candidate_count={len(search['candidates'])} "
                    f"artifact_status={artifact['status']}"
                )
            proposal = next(
                (
                    item
                    for item in artifact["proposals"]
                    if "battery video doorbell pro"
                    in " ".join(
                        filter(None, (item["brand"], item["model"], item["variant"]))
                    ).casefold()
                ),
                artifact["proposals"][0],
            )
            _expect(
                client.post(
                    f"/api/v1/projects/{project_id}/agents/competitor-discovery/artifacts/"
                    f"{artifact['artifact_id']}/decision",
                    json={
                        "action": "confirm",
                        "selected_proposal_ids": [proposal["proposal_id"]],
                        "actor": "backend-smoke-test",
                        "reason": "Confirm the exact public product candidate for onboarding.",
                    },
                ),
                200,
                "confirm live competitor candidate",
            )
            onboarding_payload = {
                "artifact_id": artifact["artifact_id"],
                "authorization_basis": "publicly_available",
                "authorization_confirmed": True,
                "authorized_by": "backend-smoke-test",
                "purpose": "Validate confirmed public competitor source onboarding.",
            }
            onboarding = _expect(
                client.post(
                    f"/api/v1/projects/{project_id}/competitor-source-onboardings",
                    json=onboarding_payload,
                ),
                201,
                "onboard confirmed competitor sources",
            )
            repeated = _expect(
                client.post(
                    f"/api/v1/projects/{project_id}/competitor-source-onboardings",
                    json=onboarding_payload,
                ),
                200,
                "repeat competitor source onboarding",
            )
            record = onboarding["onboarding"]
            evidence = _expect(
                client.get(f"/api/v1/projects/{project_id}/evidence"),
                200,
                "verify onboarding evidence boundary",
            )
            sources = _expect(
                client.get(f"/api/v1/projects/{project_id}/sources"),
                200,
                "list onboarded source assets",
            )
            source_pipeline_results = []
            for item in record["items"]:
                source_asset_id = item["source_asset"]["source_asset_id"]
                processing = _expect(
                    client.get(
                        f"/api/v1/projects/{project_id}/sources/"
                        f"{source_asset_id}/processing"
                    ),
                    200,
                    "read onboarded collection job",
                )
                routing: dict[str, Any] | None = None
                if processing["job"]["status"] == "succeeded":
                    routing = _expect(
                        client.get(
                            f"/api/v1/projects/{project_id}/sources/"
                            f"{source_asset_id}/routing"
                        ),
                        200,
                        "read automatic source routing",
                    )
                source_pipeline_results.append(
                    {
                        "source_asset_id": source_asset_id,
                        "source_url": item["source_asset"]["source_url"],
                        "processing_status": processing["job"]["status"],
                        "processing_error_code": processing["job"]["error_code"],
                        "routing_status": routing["status"] if routing is not None else None,
                        "confirmed_routes": (
                            routing["confirmed_routes"] if routing is not None else []
                        ),
                    }
                )
            requirements = _expect(
                client.get(f"/api/v1/projects/{project_id}/source-requirements"),
                200,
                "read lineage-aware source requirements",
            )

    return {
        "model_id": model_id,
        "search_status": search["status"],
        "search_candidate_count": len(search["candidates"]),
        "artifact_status": artifact["status"],
        "confirmed_product": " ".join(
            filter(None, (proposal["brand"], proposal["model"], proposal["variant"]))
        ),
        "onboarding_created": onboarding["created"],
        "idempotent_reuse": repeated["created"] is False,
        "onboarding_item_count": record["total_item_count"],
        "source_asset_count": sources["total"],
        "source_pipeline_results": source_pipeline_results,
        "source_requirements_status": requirements["status"],
        "source_requirements_input_hash": requirements["input_hash"],
        "evidence_count": evidence["total"],
    }


def main() -> None:
    if hasattr(sys.stdout, "reconfigure"):
        sys.stdout.reconfigure(encoding="utf-8")
    parser = argparse.ArgumentParser(
        description="Run live competitor discovery, confirmation and source onboarding."
    )
    parser.add_argument("--model", default="anker:deepseek-v4-pro")
    args = parser.parse_args()
    print(json.dumps(run_live_smoke(args.model), ensure_ascii=False, indent=2))


if __name__ == "__main__":
    main()
